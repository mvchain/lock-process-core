...
public class TransationService extends BaseBiz<TransactionMapper, Transaction> {
    ...
    public void withdraw(WithdrawDTO withdrawDTO, String authorization) {
        // 校验余额
        ...
        BigInteger id = insertTrans(withdrawDTO, authorization);
        // 调用提现接口,异步
        sendTrans(withdrawDTO, authorization, id);
    }

    @Async
    public void sendTrans(WithdrawDTO withdrawDTO, String authorization, BigInteger id) {
        ...
        sendTransactionDTO.setValue(CoinUtil.Value2wei(withdrawDTO.getValue(), withdrawDTO.getType()));
    ...
    public BigInteger insertTrans(WithdrawDTO withdrawDTO, String authorization) {
        ...
        transaction.setFromAddress(BaseContextHandler.getAddress());
    ...
    private void checkSms(WithdrawDTO withdrawDTO) {
        ...
        mobileValiDTO.setMobile(BaseContextHandler.getName());
...
//        Assert.isTrue(checkResult.getBody(), "验证码错误");
    ...
    private void checkWithdraw(WithdrawDTO withdrawDTO) {
        ...
        Assert.isTrue(max.subtract(null == cost ? BigInteger.ZERO : cost).subtract(value).compareTo(BigInteger.ZERO) >= 0, "限额不足");
        Assert.isTrue(balance.subtract(value).compareTo(BigInteger.ZERO) >= 0, "余额不足");

    }

    public void deposite(DepositeDTO depositeDTO) {
        // 校验密码
        Transaction transaction = new Transaction();
